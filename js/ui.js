    ...
    <header class="bg-dark-900/90 border-b border-primary-900/40 sticky top-0 z-40 backdrop-blur">
      <div class="container mx-auto px-4 max-w-7xl flex items-center justify-between h-16">
        <a href="#home" class="flex items-center gap-2 font-bold text-xl text-primary-300 hover:text-primary-200 transition">
          <span class="text-2xl">🛡️</span> Follayt
        </a>
        <nav class="hidden md:flex items-center gap-1">
          <a href="#home" class="px-3 py-2 rounded-lg hover:bg-primary-900/40 text-sm font-medium">Главная</a>
          <a href="#chats" class="px-3 py-2 rounded-lg hover:bg-primary-900/40 text-sm font-medium">Чаты</a>
          <a href="#guarantor-app" class="px-3 py-2 rounded-lg hover:bg-primary-900/40 text-sm font-medium">Стать гарантом</a>
          ${hasPermission(currentUserData, 'admin') ? `<a href="#admin" class="px-3 py-2 rounded-lg hover:bg-primary-900/40 text-sm font-medium text-amber-400">Админ</a>` : ''}
        </nav>
        <div class="flex items-center gap-3">
          ${isLogged ? `
            <a href="#profile" class="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-primary-900/40">
              <span class="font-medium text-sm">${escapeHtml(currentUserData?.nickname || 'Профиль')}</span>
              ${roleBadge}
            </a>
            <button onclick="logout()" class="text-sm text-gray-400 hover:text-white px-2">Выйти</button>
          ` : `
            <a href="#login" class="px-4 py-2 text-sm font-medium rounded-lg hover:bg-primary-900/40">Войти</a>
            <a href="#register" class="px-4 py-2 text-sm font-medium bg-primary-600 hover:bg-primary-500 rounded-lg">Регистрация</a>
          `}
        </div>
      </div>
    </header>
        ...
        <div>
          ...
          ${!isOwner && currentUser ? `
            <button onclick="startChat('${product.id}', '${product.ownerId}')" 
              class="w-full py-3 bg-primary-600 hover:bg-primary-500 rounded-xl font-semibold text-lg">
...
function renderProfile() {
  if (!currentUser || !currentUserData) return `<div class="text-center py-20">Войдите в аккаунт</div>`;
  return `
    <div class="max-w-lg mx-auto">
